...
graph TB
    ...
    subgraph "基础设施层 Infrastructure"
        ...
        T["InMemoryVectorStore<br/>进程内向量检索（角色消歧）"]
    end
...
- `InMemoryVectorStore.java`（原名 MilvusVectorStore）- 进程内向量存储：余弦相似度检索 + BM25 混合检索，生产中用于角色消歧 Layer 2；接口与 Milvus SDK 兼容，可平滑迁移到分布式向量数据库（长文本分块管线基于它实现但未接入主流程，见第 6 节）
...
| `script.generation.single-pass.enabled` | false | 是否启用单次 AI 调用（默认关闭：实测成功率低，见下方「负结果」分析） |
...
**⚠️ 负结果：为什么默认关闭「单次 AI 调用」？**

`single-pass.enabled` 默认为 `false` 不是未完成的功能，而是一次**有记录的负结果实验**（commit `f4f59a5`）：

- **现象**：开启时编排器先尝试用一次 AI 调用生成完整剧本 JSON，实测等待 ~90s 后必然失败，随后回退多步流水线——每次生成白费 ~90s。
- **原因分析**：
  1. **输出规模超限**：完整剧本 JSON（5-10 章 × 多场景 × 角色/对白/动作）即使将 `max-tokens` 提到 16384，长结构化输出仍易被截断或格式漂移，JSON 解析失败率高；
  2. **结构化输出不稳定**：中文键名 + 深层嵌套结构对模型的指令遵循要求过高，需要大量字段名变体兼容解析，长输出下难以保证；
  3. **多步流水线并行化后更快且更可控**：对白/动作生成并行化（串行 ~90s → ~20s、~113s → ~25s）后全链路约 ~135s，且每一步可独立校验、降级、部分保存——单次调用在速度与可靠性上均无优势。
- **决策**：默认直接走多步流水线；单次调用代码与自动回退逻辑保留在仓库中（可通过配置开启），作为架构演进的对照实验记录。
...
2. **剧本生成测试：**
   - 测试多步流水线模式（默认路径）
   - 测试单次 AI 调用 v2.0 分段模式（可选实验路径：需开启 `single-pass.enabled=true`）
   - 测试 AI 失败后的回退机制
   - 测试部分结果恢复
...
✨ **生成架构演进：** 多步流水线为默认路径；单次 AI v2.0 分段经实测成功率低已默认关闭（代码与开关保留，见「负结果」分析）
...
| `LongNovelProcessor` | 串联分块 → 向量化 → 向量库存储 → 检索的预处理编排入口 | 已实现 + 单测，未接入 |
...
- **未接入原因**：
  ...
  2. 引入分布式向量数据库（如 Milvus）会让向量检索从"角色消歧的可选组件"变为"生成主链路的强依赖"，增加部署复杂度；
